...
(function (){
    ...
    AutoTypeSetPicker.prototype = {
        ...
        getHtmlTpl: function (){
            ...
            for(var i = 0,rowStr = [],ri,ris = options.listMap.rowspacing;ri=ris[i++];){
                rowStr.push('<option value="'+ri+'" '+(opt["rowSpacing"] == ri ? 'checked' : '')+'>'+ri+'</option>');
            }
            return '<div id="##" class="edui-autotypesetpicker %%">' +
                 '<div class="edui-autotypesetpicker-body">' +
                    '<table >' +
                        ...
                        '<tr><td colspan="2"><input type="checkbox" name="lineHeight" '+ (opt["lineHeight"] ? "checked" : "" )+' value = "1" >行内间距：</td><td colspan="2">' +
                                '<select name="lineHeight">'+lineStr.join('')+'</select></td></tr>'+

                        '<tr><td colspan="2"><input type="checkbox" name="rowSpacing" '+ (opt["rowSpacing"] ? "checked" : "" )+' "1">段间距：</td><td colspan="2">' +
                            '<select name="rowSpacing">'+rowStr.join('')+'</select></td></tr>'+
