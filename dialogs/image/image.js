...
(function () {
    ...
    function searchImage() {
        g( "searchList" ).innerHTML = " &nbsp;图片加载中，请稍后……";
        var key = $G( "imgSearchTxt" ).value,
                type = $G( "imgType" ).value,
                url = "http://image.baidu.com/i?ct=201326592&cl=2&lm=-1&st=-1&tn=baiduimagejson&istype=2&fm=index&pv=&word=" + encodeToGb2312( key ) + type + "&" + +new Date;
        var reqCallBack = function ( data ) {
            var imgObjs = data.data, frg = document.createDocumentFragment();
            if(imgObjs.length<2) {
                g( "searchList" ).innerHTML = "<p> :( ，抱歉，没有找到图片！请重试一次！</p>";
                return;
            }
...
//                                            };
                                    ...
                                    onerror:function () {
                                        alert( "您的图片太多了，服务器要吃不消了！" )
                                    }
                                } );
                            }
                        }
                        if ( id == "imgSearch" ) {
                            g( "imgSearchTxt" ).focus();
                        }
                    }
                }
            };
    ...
    function decodeFromGb2312( str ) {
        ...
        for ( var i = 0; i < str.length; i++ ) {
            ...
            if ( c == '+' ) {
                strOut += ' ';
            } else if ( c != '%' ) {  // a,b,c,1,2等，非%开头的，直接返回本身
                strOut += c;
            } else {// %开头
                i++;
