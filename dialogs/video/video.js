...
(function(){
     ...
     function createAlignButton( ids ) {
         for ( var i = 0, ci; ci = ids[i++]; ) {
             var floatContainer = $G( ci ),
                     nameMaps = {"none":"默认对齐", "left":"向左浮动", "right":"向右浮动", "center":"独占一行"};
    ...
    function getMovie(){
        ...
        var keyword = $G("videoSearchTxt").value,
                type = $G("videoType").value,
            str="";
        ajax.request(editor.options.UEDITOR_HOME_URL +"server/submit/getMovie.php",{
            searchKey:keyword,
            videoType:type,
