...
//    te.presskey( '', 'h' );
    setTimeout( function() {
        /*去掉空白字符*/
        ua.manualDeleteFillData( editor.body );
        var color = baidu.editor.browser.ie ? '' : ';background-color: rgb(255, 0, 0); ';
...
test( 'trace 918：字体的状态反射', function() {
    ...
    equal( editor.queryCommandValue( 'fontfamily' ), '楷体', '检查字体的状态反射' );
} );
...
test( 'trace 810：闭合时设置删除线会改变文本前景色', function() {
    ...
    var color = baidu.editor.browser.ie ? '' : 'color: rgb(255, 0, 0)';
...
test( 'trace 802：为设置了字体的文本添加删除线', function() {
    ...
    equal( editor.queryCommandValue( 'fontfamily' ), '隶书' );
} );
